...
### Load local copy of the data to enhance script's reproducibility
load("therapyMonitor-data.Rda");

### Invert inverted items
dat <- invertItems(dat, c(22, 25, 28, 31));

### Specify scales
scales <- list(positiveAffect = c(3, 6, 9, 10, 13),
               negativeAffect = c(4, 5, 7, 8, 11, 12, 14),
               selfEsteem = c(21, 22, 23),
               intimacy = 24:31,
               erectionMasturbation = 47:49,
               erectionPartnerSex = 56:58,
               experienceMasturbation = c(50, 52, 53),
               experiencePartnerSex = c(59, 61, 61));

### Create scales and add to dataframe
dat <- makeScales(dat, scales);

### Aggregate erection variables
dat <- makeScales(dat, list(erectionCombined = 7:8))

### Rename time variable
dat <- rename(dat, c("Tijdstempel" = "time"));

### Convert it to the actual time format
dat$datetime <- parse_date_time(dat$time,
                                orders="dmyhMs",
                                tz="CET");

### Add rank of measurement moment
dat$measurementNumber <- order(dat$datetime);
...
tweakList <- list(positiveAffect = list(baselineMeasurements = 6,
                  ...
                  experienceMasturbation = list(baselineMeasurements = 6,
                                                ...
                                                topBounds = c(4, 6),
                                                plotLabs=list(x="Date",
                                                              y="Experience during masturbation")),
...
do.call(genlog,
        args=c(list(data=dat,
                    ...
                    timeVar='datetime',
                    yVar='intimacy'),
               list(baselineMeasurements = 6,
                    startBase = 4,
                    startTop = 6,
                    baseBounds = c(2, 5),
                    #growthRateBounds = c(-1000000, 1000000),
                    startGrowthRate = 2,
                    topBounds = c(4, 7),
                    yRange=c(1,7),
                    plotLabs=list(x="Date",
...
genlogCompleteStartValues(data=dat,
                          timeVar='datetime',
                          yVar='intimacy',
...
### Combine plots
figure10 <- grid.arrange(grobs = unlist(lapply(names(tweakList),
                                               function(x) {
                                                 if (is.list(sexTherapyResults[[x]]$genlog.tweaked)) {
                                                   return(list(sexTherapyResults[[x]]$piecewiseRegr$output$plot,
                                                               sexTherapyResults[[x]]$genlog.nonTweaked$output$plot,
                                                               sexTherapyResults[[x]]$genlog.tweaked$output$plot));
                                                 } else {
                                                   return(list(textGrob("(missing)"),
                                                               textGrob("(missing)"),
                                                               textGrob("(missing)")));
                                                 }
                                              }), recursive=FALSE),
