...
########################################################################
### Run for selection of variables
########################################################################

tweakList <- tweakList[c(3,4,7)]
...
table3 <- t(sapply(names(tweakList),
                 function(x) {
                   return(c(Dev = sexTherapyResults[[x]]$genlog.tweaked$output$deviance,
                            Rsq = sexTherapyResults[[x]]$genlog.tweaked$output$Rsq,
                            ESc = sexTherapyResults[[x]]$genlog.tweaked$output$ES1,
                            ESr = sexTherapyResults[[x]]$genlog.tweaked$output$ES2,
                            growthRate = sexTherapyResults[[x]]$genlog.tweaked$output$growthRate,
                            IP = sexTherapyResults[[x]]$genlog.tweaked$output$inflectonPoint,
...
Finally, we will show how the functions behave under random noise data obtained from three different distributions.
...
scdResultTable <- function(object, subobject=NULL) {
  ...
  if ("genlog" %IN% class(object)) {
    return(c(Dev = object$output$deviance,
             ...
             ES1 = object$output$ES1,
             ES2 = object$output$ES2,
    ...
    return(c(Dev = object$output$deviance,
             Rsq = object$output$Rsq.null,
             ES1 = object$output$Rsq.model,
             ES2 = object$output$ES,
